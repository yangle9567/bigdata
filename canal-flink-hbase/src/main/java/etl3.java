...
public class etl3 {
    public static void main(String[] args) throws Exception {
        ...
        props.put("bootstrap.servers", "10.45.4.146:9092");//props.put("bootstrap.servers", "10.45.0.171:9092");
        ...
        DataStream<SdkExposureMessage> message3 = message2.filter(new FilterFunction<SdkExposureMessage>() {
            ...
            public boolean filter(SdkExposureMessage sdkExposureMessage) throws Exception {
                if(null != sdkExposureMessage && StringUtils.isNotBlank(sdkExposureMessage.getArticleid()) && sdkExposureMessage.getDatamap() != null){
                    return true;
                }else{
                    return false;
                }
        ...
        SingleOutputStreamOperator<SdkExposureMessage> processedStream = message3.process(new ProcessFunction<SdkExposureMessage,SdkExposureMessage>() {
            ...
            public void processElement(SdkExposureMessage sdkExposureMessage, Context context, Collector<SdkExposureMessage> out) throws Exception {
                ...
                if(null != sdkExposureMessage){
                    if (sdkExposureMessage.getSys() != null && sdkExposureMessage.getSys().equals("home_recomend")) {
        ...
        SingleOutputStreamOperator<SdkExposureMessage> message_sdkhomerec = processedStream.getSideOutput(sideOutputTag).keyBy(new KeySelector<SdkExposureMessage, String>() {
            @Override
        ...
        if(message_sdkhomerec != null){
            message_sdkhomerec.print();
        }

        //5 合并首页推荐数据与其他数据，然后联查大宽表文章属性信息
        SingleOutputStreamOperator<SdkExposureMessage> message4 = processedStream.union(message_sdkhomerec).keyBy(new KeySelector<SdkExposureMessage, String>() {
            @Override
            public String getKey(SdkExposureMessage sdkExposureMessage) throws Exception {
                //System.out.println("keyBy sdkExposureMessage=" + sdkExposureMessage.toString());
