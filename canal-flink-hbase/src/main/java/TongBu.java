...
public class TongBu {
    ...
    public static void main(String[] args) throws Exception {
        ...
        sEnv.setRestartStrategy(RestartStrategies.failureRateRestart(
                ...
                Time.seconds(10) // 两次连续重启尝试的时间间隔
        ));
        // 每隔1000 ms进行启动一个检查点
        sEnv.enableCheckpointing(1000*60);
        // 高级选项：
        ...
        sEnv.setStateBackend(new FsStateBackend("hdfs://HDFS80727/bi/tmp/tongbu/checkpoint"));
        ...
        message.timeWindowAll(org.apache.flink.streaming.api.windowing.time.Time.milliseconds(secondsdelay));
        ...
        DataStream<FlatMessage> message_youhui2 = message_youhui.map(new MapFunction<FlatMessage, FlatMessage>() {
            ...
            public FlatMessage map(FlatMessage flatMessage) throws Exception {
                ...
                if(null != newFlatMessage){
                    System.out.println("-----newFlatMessage id="+newFlatMessage.getId()+" ---tablename="+newFlatMessage.getTable());
                }
        ...
        SingleOutputStreamOperator<FlatMessage> processedStream = message_youhui2.process(new ProcessFunction<FlatMessage,FlatMessage>() {
            ...
            if(null != flatMessage){
                ...
                //System.out.println("-message_youhui2--flatMessage="+flatMessage.toString());
            }
            }
        });
